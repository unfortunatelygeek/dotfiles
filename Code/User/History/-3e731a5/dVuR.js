var cartarr = JSON.parse(localStorage.getItem("BagListObj")) || [];
var wishlistItems = [];

// Function to fetch wishlist items from localStorage
function fetchWishlistItems() {
    // Logic to filter wishlist items from cartarr
    wishlistItems = cartarr.filter(item => item.isWishlist === true);
    localStorage.setItem('wishlistItems', JSON.stringify(wishlistItems));
}

// Call fetchWishlistItems function when the page loads
document.addEventListener('DOMContentLoaded', function() {
    fetchWishlistItems();

    var itemcount = wishlistItems.length;
    localStorage.setItem("itemcount", itemcount);

    var MRP = wishlistItems.reduce(function(sum, item) {
        return sum + +(item.strikedoffprice.split(" ")[1]);
    }, 0);
    localStorage.setItem("MRP", MRP);

    var amount = wishlistItems.reduce(function(sum, item) {
        return sum + +(item.price.split(" ")[1]);
    }, 0);
    localStorage.setItem("amount", amount);

    var discount = MRP - amount;
    localStorage.setItem("discount", discount);

    document.querySelector(".amount_pay").innerText = amount;
    document.querySelector(".filldiscount").innerText = "- " + discount;
    document.querySelector(".totalprice").innerText = MRP;
    document.querySelector(".pricedets").innerText = `PRICE DETAILS ( ${itemcount} Items)`;

    wishlistItems.forEach(function(item, index) {
        var box = document.createElement("div");
        box.className = "main";

        var imgbox = document.createElement("div");

        var image = document.createElement("img");
        image.src = item.image_url;
        imgbox.append(image);

        var detailsbox = document.createElement("div");

        var name = document.createElement("p");
        name.innerText = item.brand;
        name.style.fontSize = "20px";
        name.style.marginBottom = "-8px";

        var para = document.createElement("p");
        para.innerText = item.para;
        para.style.color = "gray";

        var price = document.createElement("span");
        price.innerText = item.price;

        var strikedprice = document.createElement("span");
        strikedprice.innerText = item.strikedoffprice;
        strikedprice.style.textDecoration = "line-through";
        strikedprice.style.color = "gray";

        var offer = document.createElement("span");
        offer.innerText = item.offer;
        offer.style.color = "red";

        var pricepara = document.createElement("p");
        pricepara.append(price, strikedprice);

        detailsbox.append(name, para, pricepara, offer);

        var buttonbox = document.createElement("div");

        var remove = document.createElement("button");
        remove.innerText = "REMOVE";
        remove.addEventListener("click", function() {
            removeitem(index);
        });

        buttonbox.append(remove);

        box.append(imgbox, detailsbox, buttonbox);

        document.querySelector(".container").append(box);
    });
});

// Function to remove item from wishlist
function removeitem(index) {
    wishlistItems.splice(index, 1);
    localStorage.setItem("wishlistItems", JSON.stringify(wishlistItems));
    window.location.href = "cart.html"; // Redirect back to cart page after removal
}

// Function to apply discount
document.querySelector(".apply").addEventListener("click", discountfun);

function discountfun() {
    var payable_amount = +(localStorage.getItem("amount"));
    var int_promo = document.querySelector("#promo").value;

    if (payable_amount > 300 && int_promo === "MYNTRA300") {
        amount = amount - 300;
        discount = discount + 300;
        localStorage.setItem("amount", amount);
        localStorage.setItem("discount", discount);
        document.querySelector(".amount_pay").innerText = amount;
        document.querySelector(".filldiscount").innerText = "- " + discount;

        document.querySelector(".apply").removeEventListener("click", discountfun);
    }
}

// Function to redirect to wishlist page
document.querySelector(".wishlistlink").addEventListener("click", sendToLiveTryOn);

function sendToLiveTryOn() {
    // Store wishlist items in localStorage and redirect to live-try-on.html
    localStorage.setItem("wishlistItems", JSON.stringify(wishlistItems));
    window.location.href = "myntra/AR-page/live-try-on.html";
}

// Event listeners for other links/buttons (not directly related to wishlist functionality)
document.querySelector(".makeorder").addEventListener("click", profile);

function profile() {
    window.location.href = "../Profile/signup.html";
}

document.getElementById('landingPage').addEventListener('click', function() {
    window.location.href = "../Landingpage/index.html";
});

document.querySelector("#second").addEventListener("click", function() {
    window.location.href = "../Profile/signup.html";
});
